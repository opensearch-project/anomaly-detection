...
public class ForecastRestApiIT extends AbstractForecastSyntheticDataTest {
    ...
    public void testRunOnce() throws Exception {
        ...
        // in 2.x, responseMap = entityAsMap(response) would cause toHits(response) to fail with
        // error: ParsingException[Failed to parse object: expecting token of type [START_OBJECT] but found [null]]
        List<SearchHit> hits = toHits(response);
        assertTrue("actual: " + hits.size(), hits.size() > 40);
        long forecastFrom = -1;
