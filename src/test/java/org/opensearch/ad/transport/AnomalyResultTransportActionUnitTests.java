...
public class AnomalyResultTransportActionUnitTests extends AbstractADTest {
    ...
    public void setUp() throws Exception {
        ...
        detector = spy(TestHelpers.randomAnomalyDetectorUsingCategoryFields(detectorId, Collections.singletonList(categoryField)));
    ...
    @SuppressWarnings("unchecked")
    public void test_doExecute_returnForSingleEntity() {
        String modelId = "modelId";
        DiscoveryNode node = mock(DiscoveryNode.class);
        when(detector.isMultientityDetector()).thenReturn(false);
        when(normalModelPartitioner.getThresholdModelId(detectorId)).thenReturn(modelId);
        when(hashRing.getOwningNode(modelId)).thenReturn(Optional.of(node));
        double[] values = new double[] { 1 };
        SinglePointFeatures feature = new SinglePointFeatures(Optional.of(values), Optional.of(values));
        doAnswer(invocation -> {
            ActionListener<SinglePointFeatures> listener = invocation.getArgument(3);
            listener.onResponse(feature);
            return null;
        }).when(featureQuery).getCurrentFeatures(eq(detector), eq(request.getStart()), eq(request.getEnd()), any());

        ActionListener<AnomalyResultResponse> listener = (ActionListener<AnomalyResultResponse>) mock(ActionListener.class);
        action.doExecute(null, request, listener);

        verify(listener).onResponse(any(AnomalyResultResponse.class));
    }
}
