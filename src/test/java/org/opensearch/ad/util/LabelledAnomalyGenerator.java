...
/**
 * Regarding benchmark data, we randomly generated synthetic data with known anomalies
 * inserted throughout the signal. In particular, these are one/two/four dimensional
 * data where each dimension is a noisy cosine wave. Anomalies are inserted into one
 * dimension with 0.003 probability. Anomalies across each dimension can be independent
 * or dependent.  We have approximately 5000 observations per data set. The data set is
 * generated using the same random seed so the result is comparable across versions.
 *
 */
